 ...
 * Simulates real session behavior: waitForIdle blocks until all in-flight
 * prompts settle (just like the real #promptInFlightCount mechanism).
 */
function createMockContext(opts?: {
	...
	// Simulate session's in-flight prompt tracking.
	let inFlightCount = 0;
	...
	const session = {
		...
		prompt: vi.fn(async (text: string) => {
			...
			try {
				...
				inFlightCount--;
				// Defer idle notification to match real async scheduling.
				queueMicrotask(checkIdle);
			}
		}),
...
/** Drain the microtask queue so .catch() handlers and idle notifications settle. */
async function flushMicrotasks() {
	for (let i = 0; i < 10; i++) await Promise.resolve();
}

describe("UiHelpers.flushCompactionQueue", () => {
	it("calls waitForIdle before each prompt to avoid AgentBusyError", async () => {
		...
		// "hello" is the firstPrompt.
		// Both should have waitForIdle before their prompt call.
		expect(calls).toEqual(["waitForIdle", "prompt:/compact", "waitForIdle", "prompt:hello"]);
	});

	it("sends followUp and steer without waitForIdle (non-blocking enqueue)", async () => {
		const { ctx, calls } = createMockContext({
			queuedMessages: [
		...
		expect(calls).toEqual(["waitForIdle", "prompt:main message", "followUp:follow up", "steer:steer msg"]);
	});

	it("stops rest processing when firstPrompt fails (batchFailed)", async () => {
		const promptError = new Error("AgentBusyError");
		const { ctx, session } = createMockContext({
			queuedMessages: [
			...
			promptImpl: async (text: string) => {
				if (text === "will fail") throw promptError;
			},
		});
		const helpers = new UiHelpers(ctx);
		...
		// firstPrompt ("will fail") rejects → batchFailed set.
		// /next calls waitForIdle which blocks until firstPrompt settles.
		// After settlement, batchFailed is true → rest loop breaks.
		...
		// Error should report firstPrompt + rest as unsent, not preCommands (none here).
		expect(errorArg).toContain("will fail");
		expect(errorArg).toContain("/next");
		expect(errorArg).toContain("also queued");

		// /next should NOT have been prompted.
	...
	it("does not double-report when outer catch fires after firstPrompt already failed", async () => {
		// With a single message that is the firstPrompt and fails, only firstPrompt.catch fires.
		// The outer catch should be suppressed by the batchFailed guard.
		const { ctx } = createMockContext({
	...
	it("willRetry path sends all messages sequentially", async () => {
		...
		expect(calls).toEqual(["waitForIdle", "prompt:/cmd", "followUp:msg"]);
		expect(ctx.showError).not.toHaveBeenCalled();
	});
});
